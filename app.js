...
function drawLabel(object) {
  ...
  text(object.label, object.x + 20, object.y + 54);
}
...
function speakDetectedObjects(objects) {
  ...
  for (let i = 0; i < objects.length; i++) {
    if (!spokenObjects.includes(objects[i].label)) {
      text += objects[i].label + ", ";
      spokenObjects.push(objects[i].label);
    }
  }
  // Speak the text if there are new objects and speech synthesis is not in progress
  if (text !== "I see " && !speechSynthesis.speaking) {
